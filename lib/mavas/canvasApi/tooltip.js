...
function importData(data) {
  ...
  const save = (data) => {
    for(let i = 0, len = data.coords.length; i < len; i ++) {
      let coords = data.coords[i];

      this.tooltip.original.push({
        coords: coords,
        lnglat: new AMap.LngLat(coords[0], coords[1]),
        size: data.size[i],
        desc: data.desc[i],
      });
    };
            ...
            for(let i = 0, len = result.length; i < len; i++) {
              drawLongText({
                ...
                origin: {
                  ...
                  y: result[len - 1].boundPixel[1] + (this.options.style.padding * 2 + this.options.style.lineHeight * Number(this.options.style.font.match(/^\d+/))) * i,
                },
                left: this.options.style.left,
                padding: this.options.style.padding,
                width: this.options.style.width,
                lineHeight: this.options.style.lineHeight,
                font: this.options.style.font,
    ...
    if (lng <= this.boundLngLat[0] && lat <= this.boundLngLat[1] && lng >= this.boundLngLat[2] && lat >= this.boundLngLat[3]) {
      ...
      this.tooltip.cache.push(
        Object.assign(
          ...
          tooltip,
          {
            boundPixel: [pixel.x + tooltip.size.width / 2, pixel.y - tooltip.size.height, pixel.x - tooltip.size.width / 2, pixel.y]
          }
        )
      );
    }
