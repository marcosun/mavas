...
function importData(data) {
  ...
  const save = (data) => {
    ...
    for(let ia = 0, lena = data.location.length; ia < lena; ia++) {
      ...
      this.polyline.original.push(newCurrentLine);
    };
  };
  
  this.polyline = {};
  this.polyline.original = [];
...
function draw(isForceRedraw) {
  ...
  */
  const drawWithDelay = () => {
    ...
    let now = new Date();
    let delayInterval = this.options.delay.interval, delaySize = this.options.delay.size, i = 0, len = this.polyline.cache.length / delaySize;
