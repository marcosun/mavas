...
/*
  *@param {String} data [polyline point lnglat]
*/
function importData(data) {
  this.polyline = {};
  this.polyline.original = data;
};
...
function draw() {
  ...
  const drawWithNoDelay = () => {
    ...
    for(let ia = 0, lena = this.polyline.cache.length; ia < lena; ia++) {
      ...
      for(let ib = 0, lenb = currentLine.length; ib < lenb; ib++) {
        ...
        this.virtualCtx.lineTo(cLngLat.x, cLngLat.y);
      };
  ...
  */
  const drawWithDelay = () => {
    ...
    let now = new Date();
    let delayInterval = this.option.delay.interval, delaySize = this.option.delay.size, i = 0, len = this.polyline.cache.length / delaySize;
...
function cacheVisiblePolyline() {
  ...
  const nineBlocksAlgorithm = () => {
    ...
    r = Math.sqrt(Math.pow(this.centreLngLat[0] - this.boundLngLat[0], 2) + Math.pow(this.centreLngLat[1] - this.boundLngLat[1], 2));
    for(let ia = 0, lena = this.polyline.original.length; ia < lena; ia++) {
      ...
      for(let ib = 1, lenb = currentLine.length; ib < lenb; ib++) {
        ...
        let d = Math.abs((A * this.centreLngLat[0] + B * this.centreLngLat[1] + C) / Math.sqrt(Math.pow(A, 2) + Math.pow(B, 2)));
