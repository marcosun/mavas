...
function draw(isForceRedraw) {
  ...
  const drawManyCurves = () => {
    let currentLine, currentPoint, previousPoint, currentPixel, previousPixel, distanceOfPoints, distanceToControlPoint, k, b, A, B, C, controlPointX, controlPointY, controlPoint;
    ...
    for(let ia = 0, lena = this.curve.cache.length; ia < lena; ia++) {
      ...
      for(let ib = 1, lenb = currentLine.length; ib < lenb; ib++) {
        ...
        distanceToControlPoint = distanceOfPoints / 3;
        k = - (previousPixel.x - currentPixel.x) / (previousPixel.y - currentPixel.y);
        b = previousPixel.y - k * previousPixel.x;
        
        A = 1 + Math.pow(k, 2);
        B = 2 * (k * b - previousPixel.x - k * previousPixel.y);
        C = Math.pow(previousPixel.x, 2) + Math.pow(previousPixel.y, 2) + Math.pow(b, 2) - 2 * b * previousPixel.y - Math.pow(distanceToControlPoint, 2);
