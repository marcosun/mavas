...
function draw(isForceRedraw) {
  ...
  const drawBorder = (topLeftPixel, width, height, lineWidth, color) => {
  ...
  const init = () => {
    ...
    for(let i = 0, len = this.infoWindow.cache.length; i < len; i++) {
      ...
      drawBorder({x: currentInfoWindow.boundPixel[0], y: currentInfoWindow.boundPixel[1]}, currentInfoWindow.style.width, currentInfoWindow.style.height, currentInfoWindow.style.borderWidth, currentInfoWindow.style.borderColor);
