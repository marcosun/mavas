...
function draw() {
  ...
  //call canvas api on screen directly
//  const paint = () => {
//    let now = new Date();
//    let textLng, textLat;
//    
...
//  }
  ...
  const paint = () => {
    ...
    if (!canvasBuffer) {
      ...
      canvasBuffer.width = this.map.getSize().width;
      canvasBuffer.height = this.map.getSize().height;
    ...
    for(let i = 0, len = this.marker.cache.length; i < len; i ++) {
      ...
      if (currentPoint.data) {
        ...
        ctxBuffer.drawImage(icon, cLngLat.x - iconWidth / 2, cLngLat.y - iconHeight);
        //position text on the centre of the icon
        switch(currentPoint.flagDescLen) {
          case 1:
            textLng = cLngLat.x - 3;
        ...
        ctxBuffer.fillText(currentPoint.flagDesc, textLng, textLat);
      }
    }
    ...
    if (this.option.fit && this.isInitialLoad) {
      setFit();
    };
