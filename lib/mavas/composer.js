...
export default class Composer extends Component {
    ...
    case 'quadraticCurve':
      ...
      */
      options = {
        ...
        id: outerOptions.id,
        algo: this.algoReducer(outerOptions.algo),
        lineStyle: this.lineStyleReducer(outerOptions.lineStyle),
      };

      options = Object.assign({},
        options,
        {
          data: this.lineDataReducer(outerOptions.data, options),
        }
      );
      ...
      */
      ...
      options = Object.assign({},
        options,
        {
          data: {
            ...options.data,
            icon: this.iconReducer(options.data.icon, options),
          },
        }
      );
  ...
  */
  ...
  lineDataReducer(data, options) {
    const defaultConfig = {
      coords: [],
      symbol: this.symbolReducer(options.symbol),
      lineStyle: this.lineStyleReducer(options.lineStyle),
    };
      ...
      if (data[0] instanceof Array) {
        ...
        for(let i = 0, len = data.length; i < len; i++) {
          result.push({
            ...
            symbol: data[i].symbol ? this.symbolReducer(data[i].symbol) : defaultConfig.symbol,
            lineStyle: data[i].lineStyle ? this.lineStyleReducer(data[i].lineStyle) : defaultConfig.lineStyle,
          });
        }
  ...
  algoReducer(algo) {
    ...
    if (algo) {
      return {
        ...
        delay: algo.isRealtime === true ? this.delayReducer() : this.delayReducer(algo.delay),
